...
/*
 * @author lrt
 * CCS: constant condition switch switch语句中的常量条件
 */
public class CCSStateMachine {

	public static List<FSMMachineInstance> createCCSStateMachines(SimpleNode node, FSMMachine fsm) {
		List<FSMMachineInstance> list = new LinkedList<FSMMachineInstance>();
		List<SimpleNode> evaluationResults = new LinkedList<SimpleNode>();
		String xpath1 = ".//SelectionStatement[@Image='switch']/Expression";
	...
	private static void addFSM(List<FSMMachineInstance> list, SimpleNode node, FSMMachine fsm) {
		...
		if (Config.DTS_LANGUAGE == Config.LANG_ENGLISH) 		
			...
			fsmInstance.setDesp("警告：第" + fsmInstance.getRelatedASTNode().getBeginLine()+" 行switch语句中表达式为常量");
