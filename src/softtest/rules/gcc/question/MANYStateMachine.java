...
/**
 * author: Yx
 * 模式：打印参数过多(Question:PRINT_FORMAT)
 */

public class MANYStateMachine {
	
	
	public static List<FSMMachineInstance> createMANYStateMachines(SimpleNode node, FSMMachine fsm){
	...
	private static void addFSM(List<FSMMachineInstance> list, SimpleNode node, FSMMachine fsm) {
		...
		if (Config.DTS_LANGUAGE == Config.LANG_ENGLISH) {
			fsminstance.setDesp("Print too many parameters.");
		} else if (Config.DTS_LANGUAGE == Config.LANG_CHINESE){
			fsminstance.setDesp(node.getBeginLine() + "行打印参数过多。");
		}	
