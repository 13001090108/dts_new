 ...
 * 
 */
public class UFMStateMachine0 extends BasicStateMachine {
	...
	public static boolean checkUse(List nodes, FSMMachineInstance fsmin) {
		...
		while (it.hasNext()) {
			...
			if(node instanceof ASTPrimaryExpression) {
				ASTPrimaryExpression idNode = (ASTPrimaryExpression) node;
                if(fsmin.getStateData() instanceof Variable) {      // post condition
                    VariableNameDeclaration varDecl = idNode.getVariableDecl();
                    if(varDecl != null) {
		...
		if(node instanceof ASTPrimaryExpression) {
			...
			for(int i = 0; i < n; i++) {
				...
				if(paramAss.findChildrenOfType(ASTPrimaryExpression.class).size() != 0) {
					ASTPrimaryExpression param = (ASTPrimaryExpression) paramAss.findChildrenOfType(ASTPrimaryExpression.class).get(0);
					if(param == node) {
						paramindex = i;
						break;
					}
			...
			if(paramsNode.jjtGetParent() instanceof ASTPostfixExpression) {
				...
				if(post.jjtGetNumChildren() == 2 && post.jjtGetChild(0).jjtGetNumChildren()==0 && 
					...
					if(funcNode.isMethod() && funcNode.getMethodDecl() != null) {
						...
						if(funcDecl.getImage().equals("free"))
							return "free";
