		...
		try {
			...
			if (root == null) {
				throw new RuntimeException(
						"This is not a legal StateMachine define file.");
			}
			// 解析状态机
			fsm.loadXML(root);
				...
				if (n.getNodeName().equals("State")) {
					...
					for (int j = 0; j < tranlist.getLength(); j++) {
						...
						if (tran.getNodeName().equals("Transition")) {
							// 获得tostate
							Node tostateatt = tran.getAttributes()
									.getNamedItem("ToState");
							FSMState tostate = null;
							if (tostateatt != null) {
								tostate = fsm.getStates().get(
										tostateatt.getNodeValue());
								if (tostate == null) {
									throw new RuntimeException("State error.");
								}
							} else {
								throw new RuntimeException(
										"Transition must have a tostate.");
							}

							FSMTransition transition = fsm.addTransition(
									fromstate, tostate);
							transition.loadXML(tran);

							// 解析当前转换的所有的条件，当前支持五种条件：Domain，Scope，Xpath，Nextvex
							// ,True
							...
							for (int k = 0; k < conlist.getLength(); k++) {
								...
								} else if (con.getNodeName().equals(
										"AlwaysTrue")) {
			...
			// e.printStackTrace();
			throw new RuntimeException(
					"This is not a legal satemachine define file.",e);
