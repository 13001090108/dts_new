 ...
 * @version $Id: SourceCodePanel.java,v 1.2 2010/03/31 12:09:40 xqing Exp $
 */
...
public class SourceCodePanel extends JPanel implements ViewerModelListener
{
	...
	private void init()
	{
		...
		sourceCodeArea.setFont(new Font("Courier new", Font.PLAIN, 16));
			...
			SwingUtilities.invokeLater(new Runnable()
			{
				public void run()
				{
					try
					{
						...
						int startOffset = (sourceCodeArea.getLineStartOffset(node.getBeginFileLine() - 1) + node.getBeginColumn()) - 1;
						int end = (sourceCodeArea.getLineStartOffset(node.getEndFileLine() - 1) + node.getEndColumn());
