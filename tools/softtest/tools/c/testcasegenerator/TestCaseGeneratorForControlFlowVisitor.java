 ...
 * 2、选择FunctionDefinition节点生成相应的XPath表达式，然后Evaluate计算该表达式；
 * 3、确认节点的唯一性之后点击Generate即可生成测试用例代码
 * 4、将生成的测试用例代码拷贝到tests文件夹下的相应测试包内（softtest.test.c.cfg)
 * 5、根据编译平台需求（GCC/KEIL），更改相应的字段</p>
 * @author zys	
 * 2010-3-10
 */
public class TestCaseGeneratorForControlFlowVisitor {
...
class MainFrame4 extends JFrame implements ActionListener,
	...
	private TestCasePanel2 testcasePanel;
	 ...
	 */
	public MainFrame4() {
		super(NLS.nls("MAIN.FRAME.TITLE.CONTROLFLOW"));
		init();
	}
	...
	private void init() {
		...
		testcasePanel = new TestCasePanel2(model);
			...
			for (Object obj : model.getLastEvaluationResults()) {
				...
				try {
					...
					if(m!=null){
						...
						if(g!=null){
							gstring=g.printGraphForTestCaseGeneratorForCFG();
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
...
class TestCasePanel2 extends JPanel implements ViewerModelListener {
	private JTextArea testCaseArea;
	private ViewerModel model;
	public TestCasePanel2(ViewerModel model){
		this.model = model;
		init();
	}
	private void init() {
